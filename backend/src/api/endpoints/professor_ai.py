...
async def generate_lesson_plan(
    ...
    """Generate a detailed lesson plan for a specific topic"""
    current_user, professor = auth_data
    ...
    try:
        ...
        # Create system prompt
        system_prompt = """You are an expert in instructional design specializing in creating detailed, effective lesson plans.
        Your task is to generate a comprehensive lesson plan based on the provided parameters. The output should be in a structured JSON format."""

        # Format learning objectives
        learning_objectives_text = (
            "\n".join([f"- {obj}" for obj in request.learning_objectives])
            if request.learning_objectives
            else "to be determined based on the topic"
        )

        # Create the prompt
        user_prompt = f"""
        Create a detailed lesson plan for a {request.duration_minutes}-minute session on "{request.topic}" for the course "{course.title}".

        Lesson plan details:
        - Course: {course.title}
        - Topic: {request.topic}
        - Duration: {request.duration_minutes} minutes
        - Difficulty: {request.difficulty or "appropriate for the course level"}
        - Include activities: {request.include_activities}
        - Include resources: {request.include_resources}
        ...
        {{
          ...
          "differentiation": [
            "strategy for different learning needs 1",
            "strategy for different learning needs 2",
            ...
          ],
          "resources": [
            {{
              "title": "Resource title",
              "type": "article/video/website/etc.",
              "description": "Brief description"
            }}
          ],
        ...
        Make sure the lesson plan is pedagogically sound, engaging for students, and effectively addresses the learning objectives.
        """
