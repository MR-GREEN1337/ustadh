...
const LanguageSwitcher = ({ currentLang, onChangeLang }: { currentLang: string, onChangeLang: (lang: string) => void }) => {
  return (
    <div className="flex items-center gap-2">
      <button
        onClick={() => onChangeLang('ar')}
        className={`h-8 w-8 rounded-full flex items-center justify-center ${currentLang === 'ar' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 text-sm'}`}
      >
        العربية
      </button>
      <button
        onClick={() => onChangeLang('fr')}
        className={`h-8 w-8 rounded-full flex items-center justify-center ${currentLang === 'fr' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 text-sm'}`}
      >
        FR
      </button>
      <button
        onClick={() => onChangeLang('en')}
        className={`h-8 w-8 rounded-full flex items-center justify-center ${currentLang === 'en' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500 hover:bg-slate-200 text-sm'}`}
      >
        EN
      </button>
    </div>
...
// Header component
const Header = ({ currentLang, onChangeLang }: { currentLang: string, onChangeLang: (lang: string) => void }) => {
  return (
          ...
          <div className={`flex-1 z-10 ${directionClasses.textAlign}`}>
            ...
            <div className={isRTL ? 'rtl' : ''}>
              <h1 className="font-serif text-5xl md:text-7xl font-bold text-emerald-900 mb-4">
                <span className="block">{language === 'ar' ? 'أُستاذ' : t.hero.title}</span>
                {language !== 'ar' && <span className="text-4xl md:text-5xl text-emerald-700 block">أُستاذ</span>}
              </h1>
                  ...
                  <div className="h-48 bg-slate-50 rounded p-3 mb-3 overflow-hidden">
                    <div className="flex gap-2 mb-2">
                      ...
                      <div className="bg-emerald-100 text-emerald-800 p-2 rounded-lg rounded-tl-none text-sm max-w-[80%] shadow-sm">
                        <p className="mb-1">{language === 'ar' ? 'مرحبا! أنا أستاذك الشخصي. كيف يمكنني مساعدتك اليوم؟' : 
                          language === 'fr' ? 'Bonjour! Je suis votre tuteur personnel. Comment puis-je vous aider aujourd\'hui?' :
                    ...
                    <div className="flex gap-2 justify-end mb-2">
                      <div className="bg-blue-100 text-blue-800 p-2 rounded-lg rounded-tr-none text-sm max-w-[80%] shadow-sm">
                        <p className="mb-1">{language === 'ar' ? 'أحتاج مساعدة في حل مسألة رياضية عن المعادلات التفاضلية' : 
                          language === 'fr' ? 'J\'ai besoin d\'aide pour résoudre un problème de mathématiques sur les équations différentielles' :
                    ...
                    <div className="flex-1 ml-2">
                      <div className="h-10 flex items-center px-4 bg-slate-100 text-sm text-slate-400 rounded-full hover:bg-slate-200 cursor-text transition-colors border border-transparent hover:border-slate-300">
                        {language === 'ar' ? 'اكتب سؤالك هنا...' : 
                         language === 'fr' ? 'Écrivez votre question ici...' :
...
// Features section
const FeaturesSection = ({ language }: { language: string }) => {
  const t = translations[language as keyof typeof translations];
        ...
        <Tabs defaultValue="personalized" className="w-full max-w-5xl mx-auto">
          <TabsList className="grid w-full grid-cols-3 mb-12">
            <TabsTrigger value="personalized" className="data-[state=active]:bg-emerald-100 data-[state=active]:text-emerald-700">
              ...
              {language === 'ar' ? 'مخصص' : language === 'fr' ? 'Personnalisé' : 'Personalized'}
            </TabsTrigger>
            ...
            <TabsTrigger value="practice" className="data-[state=active]:bg-emerald-100 data-[state=active]:text-emerald-700">
              ...
              {language === 'ar' ? 'تدريب' : language === 'fr' ? 'Pratique' : 'Practice'}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="personalized" className="mt-0">
            <Card className="p-6">
              <h3 className="text-xl font-bold mb-4">
                {language === 'ar' ? 'ذكاء اصطناعي يفهمك' :
                  language === 'fr' ? 'Une IA qui vous comprend' :
          ...
          <TabsContent value="curriculum" className="mt-0">
            <Card className="p-6">
              <h3 className="text-xl font-bold mb-4">
                {language === 'ar' ? 'متوافق مع التعليم المغربي' :
                  language === 'fr' ? 'Aligné avec l\'éducation marocaine' :
          ...
          <TabsContent value="practice" className="mt-0">
            <Card className="p-6">
              <h3 className="text-xl font-bold mb-4">
                {language === 'ar' ? 'التمرين يصنع الإتقان' :
                  language === 'fr' ? 'La pratique mène à la perfection' :
...
const AppDownloadSection = ({ language }: { language: string }) => {
  const isRTL = language === 'ar';
  ...
  const t = appTranslations[language as keyof typeof appTranslations];

  // Get appropriate app store badge paths based on language
  const getAppStoreBadgePath = () => {
    const theme = 'black'; // or 'white' depending on your design
                    ...
                    <div className="bg-white h-[calc(100%-3rem)] rounded-b-lg flex flex-col p-3 shadow-sm">
                      <div className="h-20 bg-emerald-100 rounded mb-2 flex items-center justify-center">
                        <span className="text-emerald-800 font-medium">
                          {language === 'ar' ? 'أستاذ - مدرسك الشخصي' :
                            language === 'fr' ? 'Ustadh - Votre tuteur personnel' :
          ...
          <div className={`w-full md:w-1/2 ${isRTL ? 'text-right md:order-1' : ''}`}>
            <div className="max-w-lg">
              <h3 className="text-2xl font-bold text-slate-800 mb-4">
                {language === 'ar' ? 'استمر في التعلم أينما كنت' :
                  language === 'fr' ? 'Continuez à apprendre où que vous soyez' :
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                <a href="#" className={`h-14 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <img
                    ...
                    alt={`Download on the App Store - ${language}`}
                    className="h-full"
                  />
                </a>
                ...
                {/* Google Play button */}
                <a href="#" className={`h-14 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  {/* If you have localized Google Play badges, use similar approach as for App Store */}
                  <img
                    src={getGooglePlayBadgePath()}
                    alt={`Get it on Google Play - ${language}`}
                    className="h-full"
                  />
                </a>
...
const PaymentSection = ({ language }: { language: string }) => {
  const isRTL = language === 'ar';
  ...
  const t = paymentTranslations[language as keyof typeof paymentTranslations];
            ...
            <div className="flex flex-col md:flex-row justify-between items-center">
              <div className="flex flex-col items-center mb-4 md:mb-0 px-4">
                ...
                <p className="text-center text-sm">
                  {language === 'ar' ? 'اختر خطة' :
                    language === 'fr' ? 'Choisissez un plan' :
                      'Choose a plan'}
                </p>
              </div>
              ...
              <div className="flex flex-col items-center mb-4 md:mb-0 px-4">
                ...
                <p className="text-center text-sm">
                  {language === 'ar' ? 'اختر طريقة الدفع' :
                    language === 'fr' ? 'Sélectionnez le paiement' :
                      'Select payment'}
                </p>
              </div>
              ...
              <div className="flex flex-col items-center mb-4 md:mb-0 px-4">
                ...
                <p className="text-center text-sm">
                  {language === 'ar' ? 'أكمل الدفع' :
                    language === 'fr' ? 'Complétez le paiement' :
                      'Complete payment'}
                </p>
              </div>
              ...
              <div className="flex flex-col items-center px-4">
                ...
                <p className="text-center text-sm">
                  {language === 'ar' ? 'استمتع بالخدمة' :
                    language === 'fr' ? 'Profitez du service' :
                      'Enjoy the service'}
                </p>
              </div>
            </div>
...
// CTA Section
const CTASection = ({ language }: { language: string }) => {
  return (
    <section className="py-12 bg-emerald-600 text-white">
      <div className="container mx-auto px-4 text-center">
        <h2 className="text-3xl font-bold mb-6">
          {language === 'ar' ? 'ابدأ رحلتك التعليمية اليوم' :
            language === 'fr' ? 'Commencez votre parcours éducatif aujourd\'hui' :
              'Start your educational journey today'}
        </h2>
        <Button size="lg" className="bg-white text-emerald-700 hover:bg-emerald-50">
          {language === 'ar' ? 'سجل الآن' :
            language === 'fr' ? 'Inscrivez-vous maintenant' :
              'Sign up now'}
        </Button>
      </div>
    </section>
...
const Footer = ({ language, onChangeLang }: { language: string, onChangeLang: (lang: string) => void }) => {
  const isRTL = language === 'ar';
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
              ...
              <div>
                ...
                <a href="#" className="block mb-2 hover:text-emerald-400 transition">
                  {language === 'ar' ? 'المعلوميات' : language === 'fr' ? 'Informatique' : 'Computer Science'}
                </a>
                <a href="#" className="block mb-2 hover:text-emerald-400 transition">
                  {language === 'ar' ? 'العلوم الإسلامية' : language === 'fr' ? 'Études Islamiques' : 'Islamic Studies'}
                </a>
              </div>
            </div>
...
export default function Home() {
  ...
  // Add this useEffect after the language direction effect
useEffect(() => {
  ...
  return (
    <>
      ...
      <main>
        <Header currentLang={language} onChangeLang={setLanguage} />
        <HeroSection language={language} />
        <FeaturesSection language={language} />
        <PaymentSection language={language} />
        <AppDownloadSection language={language} />
        <CTASection language={language} />
        <Footer language={language} onChangeLang={setLanguage}/>
      </main>
    </>
  );
}